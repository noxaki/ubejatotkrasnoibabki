...
# Audio
var step_sound_player: AudioStreamPlayer
...
func _ready():
	...
	# Setup Audio
	step_sound_player = AudioStreamPlayer.new()
	step_sound_player.stream = load("res://assets/audio/step.wav")
	add_child(step_sound_player)
	
	jump_sound_player = AudioStreamPlayer.new()
	jump_sound_player.stream = load("res://assets/audio/jump.wav")
...
func walk_process(delta):
	...
	if direction:
		...
		time_since_last_step = STEP_INTERVAL # Reset so sound plays immediately on next move
...
func lose_life():
	Inventory.trigger_jumpscare()
	set_physics_process(false) # Stop movement
	set_process_input(false) # Stop looking around
